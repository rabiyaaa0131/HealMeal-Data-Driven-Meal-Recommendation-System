...
def get_meals(food_type):
    ...
    if food_type == "Veg":
        ...
        return {
            ...
            "Dinner (7:30–8:30 PM)": [
                ("2 chapati", 180, 30, 6, 4),
                ("Grilled chicken / fish", 250, 0, 28, 12),
...
if submit:
    ...
    # Safe Name + Condition
    user_name = name.strip() if name.strip() != "" else "User"
    main_condition = conditions[0] if len(conditions) > 0 else "General Wellness"

    # Personalized Greeting
    st.markdown(f"""
    ## 🌿 Personalized Health & Meal Guidance
    ...
    **Hello {user_name} 👋**
    ...
    ✨ Stay positive  
    ✨ Follow your meal plan regularly  
    ✨ Take proper rest and stay hydrated  

    We are here to support you towards better health 💙
    """)
    ...
    def generate_pdf():
        ...
        elements.append(Paragraph(f"Condition: {main_condition}", styles["Normal"]))
        elements.append(Paragraph(f"Food Preference: {food_pref}", styles["Normal"]))

        elements.append(Spacer(1, 10))

        elements.append(Paragraph(
            f"This meal plan is personalized for {user_name} to support better health.",
            styles["Normal"]
        ))
        ...
        elements.append(Paragraph(
            "<i>This is a student project and does not replace medical advice.</i>",
            styles["Italic"]
        ))
